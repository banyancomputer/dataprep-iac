...
- Run benchmarks on your local machine to get a baseline for performance
...
As such, the following behavior and assumptions are made. If they do not make sense, please read about configuring your environment in `env/` and come back to this section.
...
So long as you setup your inputs to reflect these assumptions, you should be able to run the benchmark process on any instance or local environment.
...
After configuring `env.benchmark`, run the following command:
```bash
# Install the latest version of your branch on the instance
./scripts/install.sh
```
It will also install `fake-file` on the instance, which is used to generate fake files for testing.
...
If you would like to clear the `INPUT_PATH` after you are done testing, you can use the `clear_input.sh` script:
```bash
# Clear the INPUT_PATH
./scripts/clear_input.sh
```

### Running the Benchmark
After:
- configuring `env.benchmark`
...
./results.sh
```
You should see them organized by instance, commit, and input in the `results` directory.
...
## Setting up IFTTT
You can configure `env.ifttt` to send you notifications when the benchmark is complete or as it progresses.
